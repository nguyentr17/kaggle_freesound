...
def train_model(x_train: NpArray, x_val: NpArray, y_train: NpArray, y_val:
    ...
    x = keras.layers.Convolution2D(48, (3,10), padding="same")(x)
    x = keras.layers.BatchNormalization()(x)
    x = keras.layers.Activation("relu")(x)
    x = keras.layers.MaxPool2D()(x)
    if DROPOUT_COEFF:
    ...
    x = keras.layers.Convolution2D(64, (2,10), padding="same")(x)
    x = keras.layers.BatchNormalization()(x)
    x = keras.layers.Activation("relu")(x)
    x = keras.layers.MaxPool2D()(x)
    if DROPOUT_COEFF:
    ...
    x = keras.layers.Convolution2D(128, (2,10), padding="same")(x)
    x = keras.layers.BatchNormalization()(x)
    x = keras.layers.Activation("relu")(x)
    x = keras.layers.MaxPool2D()(x)
    if DROPOUT_COEFF:
    ...
    model.fit(x_train, y_train, batch_size=BATCH_SIZE, epochs=NUM_EPOCHS,
              verbose=1, validation_data=[x_val, y_val], callbacks=[map3, reduce_lr])
