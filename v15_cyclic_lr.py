...
def train_model_with_params(params: Dict[str, str], name:str="nofolds") -> float:
    ...
    fc_dropout_coeff    = 0.55 # float(params["fc_dropout_coeff"])
    num_cnn_layers      = 5 # int(params["num_cnn_layers"])
    cnn_depth           = 45 # int(params["cnn_depth"])
    num_fc_layers       = 1     # int(params["num_fc_layers"])
    num_hidden          = 76 # int(params["num_hidden"])
    ...
    padding             = "same" # params["padding"]
    cnn_kern_width      = 1 # float(params["cnn_kern_width"])
    cnn_kern_height     = 1 # float(params["cnn_kern_height"])
    cnn_dim_decay       = 0.5 # float(params["cnn_dim_decay"])
    cnn_depth_growth    = 2 # float(params["cnn_depth_growth"])
    ...
    for L in range(num_cnn_layers):
        ...
        kern_h = max(int(cnn_kern_height * (cnn_dim_decay ** L)), 2)
        d = min(int(cnn_depth * (cnn_depth_growth ** L)), 100)

        if padding != "same":
            w, h = w - kern_w + 1, h - kern_h + 1
...
if __name__ == "__main__":
    ...
    if USE_HYPEROPT:
        ...
        hyperopt_space = {
            ...
            "lr_cycle_len"      : hp.qloguniform("lr_cycle_len", log(3), log(10), 1),
            "max_lr"            : hp.uniform("max_lr", -3.5, -2),
            "min_lr"            : hp.uniform("min_lr", -5.5, -4),
        }
